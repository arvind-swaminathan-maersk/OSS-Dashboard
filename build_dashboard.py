...
build_dashboard.py
==================
Reads SAP OSS Closed Cases data from "data/Closed Cases Detail.xlsx",
generates an interactive HTML dashboard, and saves it to "output/index.html".

Used by GitHub Actions to auto-rebuild the dashboard when the Excel file is updated.

Author: Copilot for Arvind Swaminathan
"""
...
# ─── Configuration ───────────────────────────────────────────────────────────
INPUT_FILE = "data/Closed Cases Detail.xlsx"
OUTPUT_DIR = "output"
...
def load_and_clean(path):
    """Load Excel file and clean columns."""
...
def compute_date_range(df):
    """Compute date range string from data."""
...
def aggregate(df):
    """Aggregate data by Workstream and compute all metrics."""
    ...
    for ws, grp in groups:
        ...
        # Cases with at least one coaching flag
        ...
        # Top coaching flag (excluding Total)
    ...
    # Sort: "All Workstreams" first, then by total_cases desc
...
def build_html(data, date_range, total_cases):
    """Build the complete interactive HTML dashboard."""
...
function renderHeatmap(ws){
  ...
  rows.forEach(r=>{
    ...
    FLAG_COLS.forEach(f=>{
      ...
      body+=`<td class="heat-cell" style="background:${heatColor(pct)}" onclick="showTip(event,'${f}',${cnt},${pct},'${r.workstream.replace(/'/g,"\\'")}')">${cnt}<br><small>${pct}%</small></td>`;
    });
...
function renderCoachingTable(r){
  ...
  let html='<thead><tr><th></th><th>Flag</th><th>What It Means</th><th style="color:#82ffb5">What To Do</th><th style="color:#ff8a8a">What To Avoid</th><th>Count</th><th>%</th></tr></thead><tbody>';
  FLAG_COLS.forEach(f=>{
...
def main():
    ...
    if not os.path.exists(INPUT_FILE):
        ...
        print("  Place your Excel file at: data/Closed Cases Detail.xlsx")
        sys.exit(1)
    ...
    print(f"  Workstreams: {len(data) - 1} + All")

    # Build HTML
    html = build_html(data, date_range, total_cases)
    ...
    print(f"\n  Dashboard generated: {OUTPUT_FILE} ({size_kb} KB)")
